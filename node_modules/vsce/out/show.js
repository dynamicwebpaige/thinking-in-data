...
function showOverview({ displayName = 'unknown', extensionName = 'unknown', shortDescription = '', versions = [], publisher: { displayName: publisherDisplayName, publisherName }, categories = [], tags = [], statistics = [], publishedDate, lastUpdated, }) {
    ...
    const versionList = (versions.slice(0, limitVersions).map(({ version, lastUpdated }) => [version, viewutils_1.formatDate(lastUpdated)]));
    const { install: installs = 0, averagerating = 0, ratingcount = 0 } = statistics.reduce((map, { statisticName, value }) => (Object.assign({}, map, { [statisticName]: value })), {});
    // Render
    console.log([
        `${displayName}`,
        ...
        '',
        'Statistics:',
        ...viewutils_1.tableView(statistics.map(({ statisticName, value }) => [statisticName, Number(value).toFixed(2)])).map(viewutils_1.indentRow),
    ]
